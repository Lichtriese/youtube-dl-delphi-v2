...
const
  ...
  CSIG_RE2 = '\b(?P<sig>[a-zA-Z0-9$]{2})\s*=\s*function\(\s*a\s*\)\s*{\s*a\s*=\s*a\.split\(\s*""\s*\)';
  CSIG_RE3 = '\bc\s*&&\s*d\.set\([^,]+\s*,\s*(?P<sig>[a-zA-Z0-9$]+)\(';
  CSIG_RE4 = '\bc\s*&&\s*d\.set\([^,]+\s*,\s*\([^)]*\)\s*\(\s*(?P<sig>[a-zA-Z0-9$]+)\(';
var
  LRegEx        : TRegEx;
begin
  result := '';
  ...
  LRegEx := TRegEx.Create(cSIG_RE3);
  if LRegEx.IsMatch(AJScript) then
  begin
    result := LRegEx.Match(AJScript).Groups[1].Value;
  end;

  LRegEx := TRegEx.Create(CSIG_RE4);
  if LRegEx.IsMatch(AJScript) then
  begin
    result := LRegEx.Match(AJScript).Groups[1].Value;
  end;

  LRegEx := TRegEx.Create(CSIG_RE2);
  if LRegEx.IsMatch(AJScript) then
  begin
    result :=  LRegEx.Match(AJScript).Groups['sig'].Value;//LRegEx.Match(AJScript).Groups[1].Value;
  end;
end;
